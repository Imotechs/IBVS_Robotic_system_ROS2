...
def launch_setup(context, *args, **kwargs):
  ...
  # Correct handling of the Robotiq gripper config path
    gripper_params = PathJoinSubstitution([
        FindPackageShare("robotiq_description"),
        "config",
        "robotiq_2f85_controllers.yaml"
    ])

    # Wait for controller_manager before spawning gripper controller
   # --- Gripper Controller Spawner (works without merging YAMLs) ---
    gripper_params = PathJoinSubstitution([
        FindPackageShare("robotiq_description"), "config", "robotiq_2f85_controllers.yaml"
    ])
    ...
    gripper_controller_spawner = Node(
        ...
        arguments=[
            ...
            # IMPORTANT: provide the type explicitly
            "--controller-type", "position_controllers/GripperActionController",
            # and pass the params file as plain string (not a list)
            "--param-file", gripper_params.perform(context),
        ],
    )
    ...
    [FindPackageShare("robot_bringup"), "worlds", "store.sdf"]
    )
