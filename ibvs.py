...
class EyeInHandQRTrackingController(Node):
    ...
    def on_twist(self, msg: TwistStamped):
        ...
        if Ad is None:
            ...
            return
        self.v_cam_filtered = (
                self.alpha * v_cam + (1 - self.alpha) * self.v_cam_filtered
            )
        v_cam = self.v_cam_filtered
        v_cam[2] += 0.02    # encourage forward convergence
        # 1. Transform to base frame (pure geometry)
        v_base = Ad @ v_cam

        # 2. Optional global scaling to make motion softer
        #v_base *= 0.7   # try 0.5–0.7 if motions are aggressive
        self.v_base_filtered = (
            0.3 * v_base + 0.7 * self.v_base_filtered
        )
        v_base = self.v_base_filtered
        ...
        v_base[3:6] *= 0.3  # keep orientation control, just weaker
